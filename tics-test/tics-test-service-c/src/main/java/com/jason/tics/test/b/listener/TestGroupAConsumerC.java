...
@Slf4j
public class TestGroupAConsumerC implements RocketMQListener<Pair<Long, String>> {
    @Override
    public void onMessage(Pair<Long, String> message) {
        log.info("【GroupA - ConsumerC】 user["+message.getKey()+"] like the content["+message.getValue()+"]");
    }
}
